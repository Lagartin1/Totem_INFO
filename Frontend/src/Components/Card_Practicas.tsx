...
import Indicador_Card from "./Indicador_Card";
import {
  IdentificationIcon,
  BuildingOffice2Icon,
  MapPinIcon,
...
export default function Card_Practicas({
  ...
  const openModal = () => {
    ...
    setIsModalOpen(true);
    fetch(`${baseUrl}/api/practicas/detalle/${id}`).catch(console.error); // Silenciosamente registra si hay un error
  };
