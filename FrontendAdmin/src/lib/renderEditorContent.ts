...
export function blocksToHtml(data: OutputData | undefined | null): string {
  ...
  const safeTextFromData = (d: any): string => {
    ...
    // common EditorJS shape: { text }
    if (typeof d.text === 'string') return escapeHtml(d.text);
    // some tools use `content` as the main field
    ...
    // items arrays inside objects (common in some tools)
    ...
    // fallback to JSON inspection but avoid [object Object]
    try {
  ...
  return (data as any).blocks
    .map((block: any) => {
      ...
      switch (type) {
        ...
        case 'embed': {
          // try different fields: html, caption, source
        ...
        default: {
          // fallback: try to extract a readable string
...
export function renderEditorContent(raw?: string | null): string {
  ...
  try {
    ...
    // No es JSON: asumimos que ya es HTML o texto plano
    return raw;
  }
}
