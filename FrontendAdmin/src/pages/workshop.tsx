...
export default function Workshop() {
  ...
  const [newWorkshopData, setNewWorkshopData] = useState<{ titulo: string; link: string; descripcion: string }>({ titulo: '', link: '', descripcion: '' }); 
        ...
        try {
          ...
          if (response.status !== 401) {
            ...
            if (responseRefresh.ok) {
              ...
              const retryResponse = await fetch('/api/admin/administrar/workshops', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  credentials: 'include',
              ...
              if (!retryResult.ok) {
                throw new Error(retryResult.message || 'Error creating workshop after token refresh');
              }
              makeToast('Workshop creado exitosamente.', 'success');
            } else {
  ...
  return (
    <main className='p-6 w-full min-h-screen '>
      ...
      <div className='bg-white shadow-md rounded-lg h-lvh flex flex-col justify-center items-center'>
        <h1 className='text-2xl font-bold mb-4'>Administracion Información Workshop</h1>
        <div className="flex flex-row justify-center items-center w-full">
          <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-4 mt-20 ml-20 mr-20 overflow-y-auto">
            ...
            {!loading && data.length > 0 ? (
              data.map((workshop) => (
                <CardWorkshop onUpdate={onUpdate} showToast={makeToast} showLoader={setLoading} key={workshop.id} {...workshop} />
              ))
            ):null}  
            {!loading && data.length > 0  ? (
            ...
            <div className="flex flex-col justify-center items-center p-4">
              <h2 className="text-xl font-semibold mb-4">Agregar Nuevo Workshop</h2>
              <button className='mt-4 bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600' onClick={() => setModal(true)}>Agregar Workshop</button>
            </div>
            {modal && (
